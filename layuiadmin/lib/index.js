...
layui.extend({
    ...
    function (a) {
        ...
        r(window);
        z = function () {
            if ("/views/user/login.html" != window.location.pathname&&"/views/user/reg.html"!= window.location.pathname) {
                var u = layui.data(e.tableName);
                if (!u[e.request.tokenName]) return location.href = "/views/user/login.html?redirect=" + encodeURIComponent(window.location.pathname)
            }

        };
        layui.link(e.base + "style/admin.css?v=" + (n.v + "-1"),
            function () {
                z();
            },
            "layuiAdmin"),
